...
###########################DAILY WEATHER GENERATOR#################################################################
  ...
  #This makes a table that spits out the latitude and longitude of where you clicked on the map 
...
output$plot_wgen <- renderPlot({
  ...
  observe({
  if (input$alert_anchor == 0)
  	return()  	
  isolate({ 
	  createAlert(session, inputId="alert_anchor1",alertId="aa1",
	  		title = "Choose Location",
	  		message="Welcome to ViRTUE, a vulnerability and climate risk assessment tool for water supply utilities in the Northeast U.S. To use this tool, move through the tabs (1. Choose Location -> 6. Climate Risk) and follow the instructions in each tab.",
	  		type="success",
	  		dismiss=TRUE,
	  		block=TRUE,
	  		append=FALSE
	  )	
	  createAlert(session, inputId="alert_anchor2",alertId="aa2",
	  		title = "Instructions",
	  		message="Choose the location of your reservoir by clicking on the box above, 'Add marker on click.' Then click on the map in the location of interest. Do not proceed to the second tab ('Generate Streamflow') until figures appear to the right, indicating the completion of this step of the analysis.",
	  		type="info",
	  		dismiss=TRUE,
	  		block=TRUE,
	  		append=FALSE
...
output$plot_hydrograph <- renderPlot({
	...
	observe({
	 if (input$info_hydro == 0)
	 	return()
	 	
	 isolate({
		  createAlert(session, inputId="info_hydro1",alertId="ih1",
		  		title = "Generate Streamflow",
		  		message="This tab calibrates a rainfall-runoff model to fit the uploaded historic flows. There is also an option to run a demonstration of a water supply system in Massachusetts.",
		  		type="success",
		  		dismiss=TRUE,
		  		block=TRUE,
		  		append=FALSE
		  ...
		  createAlert(session, inputId="info_hydro2",alertId="ih2",
		  		title = "Instructions",
		  		message="Under 'Hydrologic Data:' above you can choose to 'Upload Historic Flows' or run the tool's 'Demo.' If you upload historic flows, see instructions below for the proper formatting. Input the drainage area of your reservoir in the box provided and then click 'Choose File' to browse your computer for the text file you have created.",
		  		type="info",
		  		dismiss=TRUE,
		  		block=TRUE,
		  		append=FALSE
		  )	
		   createAlert(session, inputId="info_hydro3",alertId="ih3",
		   		title = "Historic Flows Format",
		   		message="Flows should be uploaded as a text file with 5 columns: Date (e.g. 1/1/1950) Year (e.g. 1950) Month (e.g. 1) Day (e.g. 1) INFLOW (e.g. 2110). Flows should be uploaded in cubic feet per second (cfs). Dates must fall within the range of 1949 to 2009, and must start on January 1st (1/1/...).",
		   		type="warning",
		   		dismiss=TRUE,
		   		block=TRUE,
		   		append=FALSE
    ...
    output$ui <- renderUI({
      ...
      if(input$input_type=="Upload Historic Flows"){
          fileInput('file4','Historic Flows',multiple=FALSE,accept=c('text', 'text/plain'))
...
output$plot <- renderPlot({
	...
	observe({
	 ...
	 isolate({
		  createAlert(session, inputId="info_sys1",alertId="is1",
		  		title = "Water Supply Performance",
		  		message="In tabs 1 & 2, time series of precipitation and temperature were used to drive a hydrologic model which generated sequences of streamflow. These streamflow sequences are now used to explore water supply performance.",
		  		type="success",
		  		dismiss=TRUE,
		  		block=TRUE,
		  		append=FALSE
		  ...
		  createAlert(session, inputId="info_sys2",alertId="is2",
		  		title = "Instructions",
		  		message="Under 'Water Supply Demands:' above you can choose to 'Upload Water Supply Demands' or run the tool's 'Demo.' If you upload water supply demands, see instructions below for the proper formatting. Then click 'Choose File' to browse your computer for the text file you have created.",
		  		type="info",
		  		dismiss=TRUE,
		  		block=TRUE,
		  		append=FALSE
		  )	
		   createAlert(session, inputId="info_sys3",alertId="is3",
		   		title = "Water Supply Demands Format",
		   		message="Water supply demands should be uploaded as a text file with 3 columns: MONTH (e.g. 1) DAY (e.g. 1) WITHDRAWAL (e.g. 36). Demands should be uploaded in millions of gallons per day (mgd). Daily demands for 1 full year (except month/day 2/29) should be included in the text file.",
		   		type="warning",
		   		dismiss=TRUE,
		   		block=TRUE,
		   		append=FALSE
		   )	
       createAlert(session, inputId="info_sys4",alertId="is4",
          title = "Hedge",
          message="Clicking the 'Hedge' box introduces a hedging rule, designed to prevent a large, single shortage event from occurring by only releasing a percentage of total water supply demands when storages drop below specified thresholds. Storage thresholds differ throughout the year and are currently modeled after the Springfield Water and Sewer Commission's Drought Management Plan.",
          type="warning",
          dismiss=TRUE,
          block=TRUE,
          append=FALSE
   ...
   output$ui2 <- renderUI({
      ...
      if(input$input_type2=="Upload Water Supply Demands"){
          fileInput('file2','Water Supply Demands',multiple=FALSE,accept=c('text', 'text/plain'))
...
output$plot_gcms <- renderPlot({
    ...
    createAlert(session, inputId="info_gcm1",alertId="gcm1",
        title = "Instructions",
...
output$summary <- renderPlot({
 ...
 createAlert(session, inputId="info_summary1",alertId="sum1",
      title = "Instructions",
      message="No input is required here. Wait approximately 10 seconds for a plot to appear. Click on the plot if you would like more information.",
      type="info",
      dismiss=TRUE,
      block=TRUE,
      append=FALSE
...
#FINAL PLOT
    ...
    zlim <- c(0,1)
		...
		plot.axes = { points(DELTA_TEMP,DELTA_PRCP*100,pch=16);
		contour(ygrid,xgrid,CONTOUR_VARIABLE,level=c(TRESHOLD),lty=(c("solid")),add=TRUE,lwd=lwd,drawlabels=FALSE);axis(1) ;axis(2)}
		)
	}
...
output$probability <- renderPlot({
  ...
  createAlert(session, inputId="info_prob1",alertId="prob1",
      title = "Instructions",
      message="No input is required here. Wait approximately 10 seconds for a plot to appear. Click on the plot if you would like more information.",
      type="info",
      dismiss=TRUE,
