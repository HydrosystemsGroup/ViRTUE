...
shinyUI(fluidPage(
    # Application title, define various regions of user-interface
    headerPanel("ViRTUE"),
...
tags$div(
    ...
    row(
      col(
        3,
        conditionalPanel(condition = 'output.markers', 
  ...
  sidebarPanel(
      conditionalPanel(condition="input.conditionedPanels=='wgen'",
            ...
            HTML(paste(
              '<p style="text-align: center">&copy; 2014 Sarah V. Whateley</p>'
            ))             
    ),
      conditionalPanel(condition="input.conditionedPanels=='hydro'",
      		actionButton(inputId= "info_hydro","More Information"),
      		bsAlert(inputId= "info_hydro1"),
      		bsAlert(inputId= "info_hydro2"),
      		bsAlert(inputId= "info_hydro3"),
      		br(),
            ...
            radioButtons("filetype2","Select the file type",choices=list("png","pdf")),
            HTML(paste(
              '<p style="text-align: center">&copy; 2014 Sarah V. Whateley</p>'
            ))
    ),
      conditionalPanel(condition="input.conditionedPanels=='Performance'",
            actionButton(inputId= "info_sys","More Information"),
            bsAlert(inputId= "info_sys1"),
            bsAlert(inputId= "info_sys2"),
            bsAlert(inputId= "info_sys3"),
            br(),
    ...
    conditionalPanel(condition="input.conditionedPanels=='plot_gcms'",
             
             bsAlert(inputId= "info_gcm1"),
    ...
    conditionalPanel(condition="input.conditionedPanels=='summary'",
           ...
           bsAlert(inputId= "info_summary1"),
           sliderInput("rel","Reliability Threshold:",
    ...
    conditionalPanel(condition="input.conditionedPanels=='prob'", 
           bsAlert(inputId= "info_prob1"), 
           sliderInput("rel","Reliability Threshold:",
           min=0,max=1,value=0.95,step=0.01),
           checkboxInput("Standard","Hedge",FALSE),
...
mainPanel(
  ...
  tabsetPanel(id="conditionedPanels",
    ...
    tabPanel("1. Choose Location", value='wgen',tableOutput('markers'),plotOutput('test'),plotOutput("plot_wgen")), 
    tabPanel("2. Generate Streamflow", value='hydro',plotOutput("plot_hydrograph")),
    tabPanel("3. Water Supply Performance", value='Performance', plotOutput("plot",height = 700, width = 700)), 
    tabPanel("4. Climate Change Projections", value='plot_gcms',plotOutput("plot_gcms")), 
    tabPanel("5. Stress Test Results", value='summary',plotOutput("summary",height = 600, width = 700)), 
    tabPanel("6. Climate Risk",value='prob', plotOutput("probability",height = 600, width = 700))
    ),
  ### show timer
